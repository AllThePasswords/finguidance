...
export function GuidanceRuleCard({ rule, isEditing, onEdit }: GuidanceRuleCardProps) {
  ...
  return (
    <button
      ...
      className="w-full text-left bg-base-module border border-neutral-border rounded-small p-4 hover:border-neutral-border-emphasis hover:shadow-level-0 transition-all group"
    >
      <div className="flex items-start justify-between gap-3">
        ...
        <ChevronRight className="w-4 h-4 text-neutral-border-emphasis group-hover:text-text-muted mt-1 shrink-0 transition-colors" />
      </div>
    </button>
  );
}
